...
pub fn resolve_ts_type(data_type: &DataType, type_map: &TypeCollection) -> String {
    let mut raw = match data_type {
        DataType::Reference(reference) if reference.name() == "Result" => {
            if let Some((_, data_type)) = reference.generics().first() {
                resolve_ts_type(data_type, type_map)
            } else {
                "void".to_string()
            }
        }
        ...
        DataType::Reference(reference) => {
            if reference.generics().is_empty() {
                ...
                let generics: Vec<String> = reference
                    .generics()
                    .iter()
                    .map(|(_, generic)| resolve_ts_type(generic, type_map))
                    .collect();
        ...
        _ => {
            ...
            datatype(&config, &variant, type_map).unwrap_or_else(|_| "any".to_string())
        }
    };
...
pub fn get_import_list(type_map: &TypeCollection) -> Vec<String> {
    let mut imports = type_map
        .into_iter()
